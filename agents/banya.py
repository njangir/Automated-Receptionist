...
class Assistant(Agent):
    def __init__(self, context_vars: Optional[dict] = None) -> None:
        ...
        instructions_template = """Tum ek helpful male voice assistant ho aur tumhara naam Alex hai. Tum hinglish mein baat karte ho (hindi + english mix). User {name} jo phone number {phone_number} ke saath hai, woh tumse voice ke through baat kar raha hai, chahe tum conversation ko text samjho. Tum ek stock broking firm mein owner ke roop mein kaam karte ho.
    ...
    async def end_call_and_disconnect(
        ...
        try:
            ...
            webhook_url = os.getenv("END_CALL_WEBHOOK", "http://localhost:5678/webhook/dd4b17ac-f49d-4971-a374-09a5fc3bb3aa")
    ...
    async def send_call_webhook():
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.info(f"Webhook sent successfully. Status: {response.status_code}, Response: {response.text[:200]}")
